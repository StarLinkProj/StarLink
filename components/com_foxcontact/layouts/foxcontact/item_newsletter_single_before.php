...
{
	FoxHtmlElem::create('div')->attr('id', $current->getBoxId())->classes('fox-item fox-item-checkbox control-group')->classes($current->get('classes'))->append(FoxFormRender::render('label'))->append(FoxHtmlElem::create('div')->classes('controls')->attr('style', "{$current->getStyleWidth()}{$current->getStyleHeight()}")->append(FoxHtmlElem::create('input')->attr('id', $current->getItemId())->attr('name', $current->getInputName())->attr('type', 'checkbox')->attr('value', JText::_('JYES'))->checked($current->getInputValue() === JText::_('JYES'))))->show();
}